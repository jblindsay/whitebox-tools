...
impl RasterizeStreams {
    pub fn new() -> RasterizeStreams { // public constructor
...
impl WhiteboxTool for RasterizeStreams {
    ...
    fn run<'a>(&self, args: Vec<String>, working_directory: &'a str, verbose: bool) -> Result<(), Error> {
        ...
        if verbose { println!("Reading base raster data...") };
        ...
        if !out_nodata { output.reinitialize_values(background_val); }
                        ...
                        if is_between(row_y_coord, record.points[i].y, record.points[i+1].y) {
                            ...
                            if y2 != y1 {
                                ...
                                if output.get_value(row, col) == background_val {
                                    output.set_value(row, col, (record_num+1) as f64);
                                    num_stream_cells += 1;
                                } else if output.get_value(row, col) != (record_num+1) as f64 && link_end_nodes.get_value(row, col) != 1u8 {
        ...
        for row in 0..rows {
            for col in 0..columns {
                ...
                if id != background_val && link_end_nodes.get_value(row, col) != 1u8 { //  it's a stream cell
                    is_adjacent = false;
                    for n in 0..4 {
                        row_n = row + dy4[n];
                        col_n = col + dx4[n];
                        id_n = output.get_value(row_n, col_n);
                        if id_n != id && id_n != background_val && link_end_nodes.get_value(row_n, col_n) != 1u8 {
                            is_adjacent = true;
                            break;
                        }
                    }
        ...
        if !feature_id {
            ...
            for row in 0..rows {
                for col in 0..columns {
                    ...
                    if z != background_val && z > 0f64 { //  it's a stream cell
