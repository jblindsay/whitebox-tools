...
impl ConvertNodataToZero {
    pub fn new() -> ConvertNodataToZero {
        ...
        let usage = format!(">>.*{0} -r={1} -v --wd=\"*path*to*data*\" --input=in.dep -o=NewRaster.dep", short_exe, name).replace("*", &sep);
...
impl WhiteboxTool for ConvertNodataToZero {
    ...
    fn run<'a>(&self,
        ...
        if !input_file.contains(&sep) && !input_file.contains("/") { // Windows now takes forward slash in directory names.
