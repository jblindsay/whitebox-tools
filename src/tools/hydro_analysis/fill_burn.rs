...
impl FillBurn {
    pub fn new() -> FillBurn { // public constructor
        let name = "FillBurn".to_string();
        let toolbox = "Hydrological Analysis".to_string();
        let description = "Burns streams into a DEM using the FillBurn (Saunders, 1999) method.".to_string();
...
impl WhiteboxTool for FillBurn {
    ...
    fn run<'a>(&self, args: Vec<String>, working_directory: &'a str, verbose: bool) -> Result<(), Error> {
        ...
        let streams = Shapefile::new(&streams_file, "r")?;
        ...
        let vals = vec![ vec![ 0u8, 0u8, 0u8, 1u8, 1u8, 1u8 ], vec![ 0u8, 0u8, 0u8, 1u8, 1u8 ], 
            vec![ 0u8, 0u8, 0u8, 1u8, 1u8, 1u8 ], vec![ 0u8, 0u8, 0u8, 1u8, 1u8 ],
            vec![ 0u8, 0u8, 0u8, 1u8, 1u8, 1u8 ], vec![ 0u8, 0u8, 0u8, 1u8, 1u8 ],
            vec![ 0u8, 0u8, 0u8, 1u8, 1u8, 1u8 ], vec![ 0u8, 0u8, 0u8, 1u8, 1u8 ] ];
            ...
            for n in 0..8 {
                ...
                if in_queue.get_value(row_n, col_n) == 0u8 {
                    if dem.get_value(row_n, col_n) == nodata {
                        ...
                        minheap.push(GridCell{ row: row_n, column: col_n, priority: output.get_value(row_n, col_n) });
                    }
        ...
        while !minheap.is_empty() {
            ...
            for n in 0..8 {
                ...
                if in_queue.get_value(row_n, col_n) == 0u8 {
                    ...
                    if zout_n != nodata {
                        if zout_n < (zout + small_num) { zout_n = zout + small_num; } // We're in a depression. Raise the elevation.
                        output.set_value(row_n, col_n, zout_n);
                        minheap.push(GridCell{ row: row_n, column: col_n, priority: zout_n });
                    }
                    in_queue.set_value(row_n, col_n, 1u8);
                }
            }
