...
impl RaiseWalls {
    ...
    pub fn new() -> RaiseWalls { 
        ...
        let description = "Raises walls in a DEM along a line or around a polygon, e.g. a watershed.".to_string();
...
impl WhiteboxTool for RaiseWalls {
    ...
    fn run<'a>(&self, args: Vec<String>, working_directory: &'a str, verbose: bool) -> Result<(), Error> {
            ...
            for col in 0..columns {
                if walled.get_value(row, col) == 1u8 {
                    ...
                    if walled.get_value(row-1, col+1) == 1u8 {
                        if walled.get_value(row-1, col) == 0u8 && walled.get_value(row, col+1) == 0u8 {
                    ...
                    // - x o
                    if walled.get_value(row+1, col+1) == 1u8 {
                        if walled.get_value(row, col+1) == 0u8 && walled.get_value(row+1, col) == 0u8 {
        ...
        if !breach_file.trim().is_empty() {
            ...
            let breach_data = Shapefile::new(&breach_file, "r")?;
