...
use crate::lidar::*;
use crate::na;
use crate::structures::{DistanceMetric, FixedRadiusSearch3D, Point2D, RectangleWithData};
use crate::tools::*;
use crate::vector::*;
...
impl WhiteboxTool for LidarRooftopAnalysis {
    ...
    fn run<'a>(
                ...
                Ok(mut input) => {
                        ...
                        thread::spawn(move || {
                                ...
                                (0..n_points).filter(|point_num| point_num % num_procs == tid)
                            {
                                ...
                                if !p.withheld()
                                    ...
                                    && p.is_late_return()
                                    && (p.classification() < 2 || p.classification() > 5)
                                {
                                    point_in_poly = false;
                                    building_rec = 0usize;
                    ...
                    for p in 0..n_points {
                        if point_in_building[p] {
                            ...
                            frs.insert(pd.x, pd.y, pd.z, point_num);
                            point_num += 1
                        }
        ...
        let n_points = las_points.len();
        let num_points: f64 = (las_points.len() - 1) as f64; // used for progress calculation only
