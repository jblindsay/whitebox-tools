...
impl BlockMinimum {
    pub fn new() -> BlockMinimum { // public constructor
        let name = "BlockMinimum".to_string();
        
        let description = "Creates a block-minimum raster from an input LAS file.".to_string();
...
.*{0} -r={1} --wd=\"*path*to*data*\" -i=file.las -o=outfile.dep --resolution=5.0 --palette=light_quant.plt", short_exe, name).replace("*", &sep);
        ...
        BlockMinimum { name: name, description: description, parameters: parameters, example_usage: usage }
    }
}
