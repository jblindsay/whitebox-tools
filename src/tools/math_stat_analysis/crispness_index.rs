...
NOTES: This index is taken from Lindsay (2006) Sensitivity of channel mapping techniques to uncertainty in digital
elevation data. Please note that there is an error in the original published equation. Specifically, the denominator
read:

Sigma_pij(1 - p_bar)^2 + p_bar^2 (RC - Sigma_pij)
...
impl CrispnessIndex {
    pub fn new() -> CrispnessIndex { // public constructor
...
>>.*{0} -r={1} --wd=\"*path*to*data*\" -o=crispness.html", short_exe, name).replace("*", &sep);
    
        CrispnessIndex { name: name, description: description, parameters: parameters, example_usage: usage }
    }
}
...
impl WhiteboxTool for CrispnessIndex {
    ...
    fn run<'a>(&self, args: Vec<String>, working_directory: &'a str, verbose: bool) -> Result<(), Error> {
        ...
        let denominator = sum * (1f64 - mean) * (1f64 - mean) + (num_cells as f64 - sum) * mean * mean;
...
ranging from 0-1. The input image contains values outside this range.");
        ...
        writer.write_all(format!("<p>Input file: {}</p>", input_file).as_bytes())?;
        ...
        </tr>", format!("{:.*}", 4, total_dev)).as_bytes())?;

        writer.write_all(&format!("<tr>
            <td><em>SS<sub>B</sub></em></td>
            <td class=\"numberCell\">{}</td>
        </tr>", format!("{:.*}", 4, denominator)).as_bytes())?;
    
        writer.write_all(&format!("<tr>
            <td><em>C</em></td>
            <td class=\"numberCell\">{}</td>
        </tr>", format!("{:.*}", 4, crispness)).as_bytes())?;

        writer.write_all("</table>".as_bytes())?;

        if warning {
            writer.write_all("<p>WARNING: This tool is intended to be applied to membership probability (MP) rasters, with probability values 
ranging from 0-1. The input image contains values outside this range.</p>".as_bytes())?;
        }

        writer.write_all("<p><em>C</em> = <em>SS</em><sub>mp</sub> &#8725; <em>SS</em><sub>B</sub> = 
        [&sum;<sup><em>R</em></sup><sub><em>i</em>=1</sub>&sum;<sup><em>C</em></sup><sub><em>j</em>=1</sub>(<em>p<sub>ij</sub></em> 
        &minus; <span style=\"text-decoration: overline\"><em>p</em></span>)<sup>2</sup>] &#8725; 
        [ &sum;<sup><em>R</em></sup><sub><em>i</em>=1</sub>&sum;<sup><em>C</em></sup><sub><em>j</em>=1</sub> 
        <em>p<sub>ij</sub></em>(1 &minus; <span style=\"text-decoration: overline\"><em>p</em></span>)<sup>2</sup> + 
        (<em>RC</em> &minus; &sum;<sup><em>R</em></sup><sub><em>i</em>=1</sub>&sum;<sup><em>C</em></sup><sub><em>j</em>=1</sub>)]</p>".as_bytes())?;
        ...
        average probability value, and <em>i</em> and <em>j</em> refer to a cell within the image.</p>".as_bytes())?;
