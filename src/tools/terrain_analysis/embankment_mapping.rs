...
impl EmbankmentMapping {
    pub fn new() -> EmbankmentMapping {
        ...
        parameters.push(ToolParameter {
            ...
            description:
                "Minimum road width; this is the width of the paved road surface (in map units).".to_owned(),
            parameter_type: ParameterType::Float,
        ...
        parameters.push(ToolParameter {
            ...
            description:
                "Spillout slope (in degrees).".to_owned(),
            parameter_type: ParameterType::Float,
...
impl WhiteboxTool for EmbankmentMapping {
    ...
    fn run<'a>(
        ...
        let mut roads: Array2D<i8> = Array2D::new(rows, columns, 0i8, -1i8).expect("Error creating Array2D.");
        ...
        if verbose {
            println!("Time for transportation vector rasterization: {}", elapsed_time);
        }
        ...
        let dist_array = [diag_dist, dem.configs.resolution_x, diag_dist, dem.configs.resolution_y, diag_dist, dem.configs.resolution_x, diag_dist, dem.configs.resolution_y];
        ...
        let mut distance: Array2D<f64> = Array2D::new(rows, columns, -1f64, -1f64).expect("Error creating Array2D.");
        let mut seed_elev: Array2D<f64> = Array2D::new(rows, columns, -1f64, -1f64).expect("Error creating Array2D.");
        let mut max_abs_slope: Array2D<f64> = Array2D::new(rows, columns, 0f64, -1f64).expect("Error creating Array2D.");
                ...
                if roads.get_value(row, col) > 0 {
                    ...
                    for n in 0..dx_seed.len() {
                        ...
                        if roads.get_value(row+dy_seed[n], col+dx_seed[n]) <= 0 {
                            if zn > maxval && output.get_value(row+dy_seed[n], col+dx_seed[n]) != 1f64 { // Notice this AND condition? It's important, otherwise you will lose many possible seeds.
                                maxval = zn;
                                max_point = [row+dy_seed[n], col+dx_seed[n]]; // set maxPoint as the cell of maximum elevation
                            }
                        }
                    }

                    output.set_value(max_point[0], max_point[1], 1f64);
                    pqueue_dist.push(CellInfo{ row: max_point[0], col: max_point[1], distance: 0.0f64 });
                    ...
                    pqueue.push(CellInfo{ row: max_point[0], col: max_point[1], distance: 0.0 });
            ...
            for n in 0..8 {
                ...
                if zn != nodata && distance.get_value(r+dy[n], c+dx[n]) < 0.0 {
                    ...
                    if dist < max_width {
                        ...
                        embankment_slope = (embankment_height / (cell.distance+dist_array[n])).atan();
                        max_abs_slope.set_value(r+dy[n], c+dx[n], embankment_slope.abs().to_degrees().max(max_abs_slope.get_value(r, c)));
                        pqueue_dist.push(CellInfo{ row: r+dy[n], col: c+dx[n], distance: dist });
        ...
        if remove_embankments {
            ...
            let out_dem_str = output_file.replace(&format!(".{}", file_ext), &format!("_dem.{}", file_ext));
