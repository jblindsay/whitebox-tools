...
impl WhiteboxTool for RemoveOffTerrainObjects {
    ...
    fn run<'a>(
        ...
        let mut erosion: Array2D<f32> = Array2D::new(rows, columns, 0f32, nodata).expect("Error creating Array2D.");  // 2X
        for r in 0..rows {
        ...
        let mut opening: Array2D<f32> = Array2D::new(rows, columns, 0f32, nodata).expect("Error creating Array2D."); // 3X
        let mut tophat: Array2D<f32> = Array2D::new(rows, columns, 0f32, nodata).expect("Error creating Array2D."); // 4X
        for r in 0..rows {
            let data = rx.recv().expect("Error receiving data from thread.");
        ...
        let mut out: Array2D<f32> = Array2D::new(rows, columns, initial_value, nodata).expect("Error creating Array2D"); // 3X
