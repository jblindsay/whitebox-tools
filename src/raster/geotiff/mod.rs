...
pub fn write_geotiff<'a>(r: &'a mut Raster) -> Result<(), Error> {
    ...
    let image_len = (r.configs.rows as usize * r.configs.columns as usize *
                     total_bytes_per_pixel + 8usize) as u32;
    ...
    ifd.push(IfdDirectory::new(TAG_IMAGEWIDTH,
                               DT_SHORT,
                               1u32,
                               r.configs.columns as u32,
                               u32_bytes.iter().cloned().collect(),
                               Endianness::LittleEndian));

    u32_bytes = unsafe { mem::transmute((r.configs.rows as u32).to_le()) };
    ifd.push(IfdDirectory::new(TAG_IMAGELENGTH,
                               DT_SHORT,
                               1u32,
                               r.configs.columns as u32,
                               u32_bytes.iter().cloned().collect(),
    ...
    ifd.push(IfdDirectory::new(TAG_COMPRESSION,
                               DT_SHORT,
                               1u32,
    ...
    let out_pi = match r.configs.photometric_interp {
        PhotometricInterpretation::Continuous |
        PhotometricInterpretation::Categorical => PI_BLACKISZERO,
    ...
    ifd.push(IfdDirectory::new(TAG_PHOTOMETRICINTERPRETATION,
                               DT_SHORT,
                               1u32,
                               out_pi as u32,
                               vec![out_pi as u8],
    ...
    ifd.push(IfdDirectory::new(TAG_STRIPOFFSETS,
                               DT_LONG,
                               r.configs.rows as u32,
                               0u32,
                               strip_offsets_as_bytes,
                               Endianness::LittleEndian));
    ...
    ifd.push(IfdDirectory::new(TAG_SAMPLESPERPIXEL,
                               DT_SHORT,
                               1u32,
                               0u32,
                               u16_bytes.iter().cloned().collect(),
                               Endianness::LittleEndian));

    u16_bytes = unsafe { mem::transmute(1u16.to_le()) };
    ifd.push(IfdDirectory::new(TAG_ROWSPERSTRIP,
                               DT_SHORT,
                               1u32,
                               1u32,
                               u16_bytes.iter().cloned().collect(),
    ...
    ifd.push(IfdDirectory::new(TAG_STRIPBYTECOUNTS,
                               DT_LONG,
                               r.configs.rows as u32,
                               0u32,
                               strip_byte_count_as_bytes,
                               Endianness::LittleEndian));

    let software: String = String::from("whitebox-tools");
    let software_len = software.len() as u32;
    ifd.push(IfdDirectory::new(TAG_SOFTWARE,
                               DT_ASCII,
                               software_len,
                               0u32,
                               software.into_bytes(),
