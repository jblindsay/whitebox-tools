...
fn main() {
        ...
        if vec[0].to_lowercase() == "-i" || vec[0].to_lowercase() == "--i" {
          ...
          vec[0].to_lowercase() == "--h" {
            let mut s: String = "Help:\n".to_owned();
             ...
             s.push_str("--azimuth    Optional azimuth in degrees of illumination source (default is 315.0).\n");
             s.push_str("--altitude   Optional altitude in degrees of illumination source (efault is 30.0).\n");
    ...
    lidar_normal_vec(input_file, output_file, azimuth, altitude, min_points, verbose);
}

fn lidar_normal_vec(input_file: String, output_file: String, az: f64, at: f64, min_points: usize, verbose: bool) {
    let azimuth = (az - 90f64).to_radians();
    let altitude = at.to_radians();
    ...
    if verbose {
        println!("******************************");
        println!("* Welcome to lidar_hillshade *");
        println!("******************************");
    }
