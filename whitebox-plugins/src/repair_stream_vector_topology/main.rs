...
///
/// The user must specify the name of the input vector stream network (`--input`) and the output file 
/// (`--output`). Additionally, a distance threshold for snapping dangling arcs (`--snap`) must be 
/// specified. This distance is in the input layer's x-y units. The tool works best on projected input 
/// data, however, if the input are in geographic coordinates (latitude and longitude), then specifying a
/// small valued snap distance is advisable. Notice that the attributes of the input layer will not be
...
fn run(args: &Vec<String>) -> Result<(), std::io::Error> {
    ...
    for fid in 0..num_polylines {
        ...
        if configurations.verbose_mode {
            progress =
                (100.0_f64 * (fid + 1) as f64 / num_polylines as f64) as usize;
            if progress != old_progress {
    ...
    for poly_id in 0..polylines.len() {
        ...
        if configurations.verbose_mode {
            progress =
                (100.0_f64 * (poly_id + 1) as f64 / polylines.len() as f64) as usize;
            if progress != old_progress {
                println!("Looking for dangling arcs: {}%", progress);
                old_progress = progress;
            }
    ...
    for poly_id in 0..polylines2.len() {
        ...
        if configurations.verbose_mode {
            progress =
                (100.0_f64 * (poly_id + 1) as f64 / polylines2.len() as f64) as usize;
            if progress != old_progress {
                println!("Looking for dangling arcs: {}%", progress);
                old_progress = progress;
            }
