...
class WbRunner(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.filemenu.add_command(label="Suppress Tool Output", command=self.update_verbose)
        self.filemenu.add_command(label=f"max_procs={wbt.get_max_procs()}", command=self.set_max_procs)
    ...
    def update_verbose(self):
        if wbt.get_verbose_mode():
            ...
            self.filemenu.entryconfig(4, label = "Capture Tool Output")
        else:
            wbt.set_verbose_mode(True)
            self.filemenu.entryconfig(4, label = "Suppress Tool Output")

    def set_max_procs(self):
        max_procs = simpledialog.askinteger(
            "Set Maximum Processors", 
            "What should be the maximum number processors used?"
        )
        if max_procs >= -1:
            wbt.set_max_procs(max_procs)
            self.filemenu.entryconfig(5, label = f"max_procs={wbt.get_max_procs()}")
    ...
    def run_tool(self):
        ...
        if wbt.run_tool(self.tool_name, args, self.custom_callback) == 1:
            self.custom_callback("Error running {}".format(self.tool_name))
            ...
            if not wbt.get_verbose_mode():
                self.custom_callback("Done!")
